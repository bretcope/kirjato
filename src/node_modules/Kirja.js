...
function mdImageRenderer (href, title, text)
{
	...
	var simpleFormat = /^(\d+(?:\.\d+)?)?(&gt;|&lt;)?;/.exec(text);
	var style = '';
	var figureClass = '';
	if (simpleFormat)
	{
		text = text.substr(simpleFormat[0].length).trim();

		var width = Math.min(Number(simpleFormat[1]), 100);
		if (width)
			style += 'width:' + width + '%;';
	...
	return '<figure style="'+ style +'" class="'+ figureClass +'"><img src="'+ href +'" title="'+ title +'">'+ caption +'</figure>';
}
